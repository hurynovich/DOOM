...
void R_GenerateComposite(int texnum) {
  ...
  byte *block = Z_Malloc(texturecompositesize[texnum], PU_STATIC, &texturecomposite[texnum]);
  ...
  for (i = 0, patch = texture->patches; i < texture->patchcount; i++, patch++) {
    ...
    for (; x < x2; x++) {
      ...
      column_t *patchcol = (column_t *)((byte *)realpatch + LONG(realpatch->columnofs[x - x1]));
...
void R_InitTextures(void) {
  ...
  char *name_p = names + 4;
  int *patchlookup = alloca(nummappatches * sizeof(*patchlookup));
  ...
  int *maptex = maptex1 = W_CacheLumpName("TEXTURE1", PU_STATIC);
  int numtextures1 = LONG(*maptex);
  int maxoff = W_LumpLength(W_GetNumForName("TEXTURE1"));
  ...
  int temp1 = W_GetNumForName("S_START");  // P_???????
