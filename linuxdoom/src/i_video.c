...
// TrueColor palette lookup table
...
void I_GetEvent(void) {
  ...
  switch (X_event.type) {
    ...
    case ButtonPress:
      event.type = ev_mouse;
      event.data1 = (X_event.xbutton.state & Button1Mask) | (X_event.xbutton.state & Button2Mask ? 2 : 0) | (X_event.xbutton.state & Button3Mask ? 4 : 0) |
    ...
    case MotionNotify:
      event.type = ev_mouse;
      event.data1 = (X_event.xmotion.state & Button1Mask) | (X_event.xmotion.state & Button2Mask ? 2 : 0) | (X_event.xmotion.state & Button3Mask ? 4 : 0);
...
void I_FinishUpdate(void) {
  ...
  if (X_visualinfo.class == PseudoColor) {
    if (multiply == 2) {
    ...
    // TrueColor blitter with optional scaling
    int x, y, ix, iy;
    byte* src = screens[0];
    ...
    if (X_visualinfo.depth >= 24) {
      ...
      for (y = 0; y < SCREENHEIGHT; y++) {
        for (iy = 0; iy < multiply; iy++) {
          uint32_t* row_dest = dest + (y * multiply + iy) * X_width;
          for (x = 0; x < SCREENWIDTH; x++) {
            uint32_t color = palette_rgb[src[y * SCREENWIDTH + x]];
            for (ix = 0; ix < multiply; ix++) {
              *row_dest++ = color;
            }
          }
        }
      ...
      for (y = 0; y < SCREENHEIGHT; y++) {
        for (iy = 0; iy < multiply; iy++) {
          uint16_t* row_dest = dest + (y * multiply + iy) * X_width;
          for (x = 0; x < SCREENWIDTH; x++) {
            uint16_t color = (uint16_t)palette_rgb[src[y * SCREENWIDTH + x]];
            for (ix = 0; ix < multiply; ix++) {
              *row_dest++ = color;
            }
          }
...
void UploadNewPalette(Colormap cmap, byte* palette) { 
  ...
  // Initialize the colormap if in PseudoColor mode
  if (X_visualinfo.class == PseudoColor && X_visualinfo.depth == 8) { 
    ...
    // For TrueColor, update our software palette lookup table
    for (i = 0; i < 256; i++) {
      uint32_t r = gammatable[usegamma][*palette++];
      uint32_t g = gammatable[usegamma][*palette++];
      uint32_t b = gammatable[usegamma][*palette++];
      
      // Calculate pixel value based on X server's RGB masks
      palette_rgb[i] = ((r << 8 | r) * X_visualinfo.red_mask / 65535 & X_visualinfo.red_mask) |
                       ((g << 8 | g) * X_visualinfo.green_mask / 65535 & X_visualinfo.green_mask) |
                       ((b << 8 | b) * X_visualinfo.blue_mask / 65535 & X_visualinfo.blue_mask);
    }
  }
} 
...
void I_InitGraphics(void) {
  ...
  if (!XMatchVisualInfo(X_display, X_screen, 8, PseudoColor, &X_visualinfo)) {
    // If PseudoColor 8 is not available, try TrueColor
    if (!XMatchVisualInfo(X_display, X_screen, 32, TrueColor, &X_visualinfo)) {
      if (!XMatchVisualInfo(X_display, X_screen, 24, TrueColor, &X_visualinfo)) {
        if (!XMatchVisualInfo(X_display, X_screen, 16, TrueColor, &X_visualinfo)) {
          I_Error("xdoom: No compatible PseudoColor or TrueColor visuals found");
        }
      }
    }
  }
  ...
  if (grabMouse)
    XGrabPointer(X_display, X_mainWindow, True, ButtonPressMask | ButtonReleaseMask | PointerMotionMask, GrabModeAsync, GrabModeAsync, X_mainWindow, None,
                 CurrentTime);
  ...
  if (doShm) {
    ...
    // create the image
    image = XShmCreateImage(X_display, X_visual, X_visualinfo.depth, ZPixmap, 0, &X_shminfo, X_width, X_height);
    ...
    image = XCreateImage(X_display, X_visual, X_visualinfo.depth, ZPixmap, 0, (char*)malloc(image->bytes_per_line * X_height), X_width, X_height, 32, 0);
  }

  if (X_visualinfo.class == PseudoColor && multiply == 1)
